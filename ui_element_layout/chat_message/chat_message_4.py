...
def chat_page():
    ...
    with ui.element('div').style(
        'display: grid; grid-template-rows: 1fr auto; height: 100vh; width: 100%; overflow: hidden; margin: 0; padding: 0;'
    ):
        
        # 消息展示区域 - 第一行，占据剩余空间
        with ui.column().classes('overflow-y-auto p-4 w-full rounded').style(
        ...
        with ui.element('div').style('padding: 10px;'):
            with ui.row().classes('w-full items-center gap-2 p-3 rounded bg-white shadow-lg border'):
                # 提前声明可变对象，供内部嵌套函数读写
                ...
                async def handle_message():
                    ...
                    # 使用JavaScript直接滚动到底部
                    ui.run_javascript(f'''
                        const scrollElement = document.querySelector('[data-testid="{scroll_area.id}"]');
                        if (scrollElement) {{
