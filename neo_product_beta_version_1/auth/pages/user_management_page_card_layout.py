    ...
    with ui.card().classes('w-full mt-6'):
        ...
        def load_users():
            ...
            with users_container:
                ...
                # 显示搜索结果统计
                if search_term:
                    ui.label(f'搜索 "{search_term}" 找到 {len(users)} 个用户').classes('text-sm text-gray-600 dark:text-gray-400 mb-4')

                # 创建用户卡片网格 - 每行2个（与角色页面保持一致）
                for i in range(0, len(users), 2):
                    with ui.row().classes('w-full gap-3'):
                        # 第一个用户卡片
                        with ui.column().classes('flex-1'):
                            create_user_card(users[i])
                        
                        # 第二个用户卡片（如果存在）
                        if i + 1 < len(users):
                            with ui.column().classes('flex-1'):
                                create_user_card(users[i + 1])
                        else:
                            # 如果是奇数个用户，添加占位符保持布局
                            ui.column().classes('flex-1')

        def create_user_card(user_data: DetachedUser):
            """创建单个用户卡片 - 与角色卡片风格保持一致"""
            # 确定用户状态主题
                ...
                with ui.row().classes('w-full p-4 gap-4'):
                    ...
                    with ui.column().classes('flex-none w-80 gap-2'):
                        ...
                        with ui.row().classes('items-center gap-3 mb-2'):
                            ui.icon('person').classes(f'text-3xl {icon_theme}')
                            with ui.column().classes('gap-0'):
                                ui.label(user_data.username).classes('text-xl font-bold text-gray-800 dark:text-gray-200')
                                ui.label(user_data.email).classes('text-sm text-gray-500 dark:text-gray-400')
                        ...
                        # 用户详细信息
                        ui.label('详细信息:').classes('text-xs font-medium text-gray-600 dark:text-gray-400')
                        if user_data.full_name:
                            ui.label(f'姓名: {user_data.full_name}').classes('text-sm text-gray-700 dark:text-gray-300')
                        if user_data.phone:
                            ui.label(f'电话: {user_data.phone}').classes('text-sm text-gray-700 dark:text-gray-300')

                        # 统计信息
                        with ui.row().classes('gap-2 mt-2'):
                            with ui.card().classes('flex-1 p-2 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600'):
                                ui.label('用户ID').classes('text-xs text-gray-500 dark:text-gray-400')
                                ui.label(str(user_data.id)).classes('text-lg font-bold text-blue-600 dark:text-blue-400')
                            
                            with ui.card().classes('flex-1 p-2 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600'):
                                ui.label('登录次数').classes('text-xs text-gray-500 dark:text-gray-400')
                                ui.label(str(user_data.login_count)).classes('text-lg font-bold text-green-600 dark:text-green-400')
                    ...
                    with ui.column().classes('flex-1 gap-2'):
                        ...
                        # 角色展示区域
                        with ui.card().classes('w-full p-3 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 min-h-[100px]'):
                            if user_data.roles:
                                ...
                                with ui.column().classes('w-full items-center justify-center py-4'):
                                    ui.icon('security_update_warning').classes('text-3xl text-gray-400 mb-1')
                        ...
                        with ui.card().classes('w-full p-3 bg-gray-50 dark:bg-gray-600 border border-gray-200 dark:border-gray-600'):
                            ...
                            ui.label(format_datetime(user_data.last_login) if user_data.last_login else '从未登录').classes('text-sm text-gray-800 dark:text-gray-200')
                        ...
                        with ui.row().classes('gap-1 w-full mt-2'):
                            ...
                            ui.button('重置密码', icon='lock_reset',
                                    on_click=lambda u=user_data: safe(lambda: reset_password_dialog(u.id))).classes('flex-1 bg-orange-600 hover:bg-orange-700 text-white py-1 text-xs')
                            ...
                            if not auth_manager.current_user or auth_manager.current_user.id != user_data.id:
                                ...
                                ui.button('当前用户', icon='person',
                                        on_click=lambda: ui.notify('不能删除当前登录用户', type='info')).classes('flex-1 bg-gray-400 text-white py-1 text-xs').disable()
