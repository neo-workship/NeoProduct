...
def chat_page():
    ...
    with ui.column().classes('w-full h-screen flex flex-col').style('overflow: hidden; height: 100vh; margin: 0; padding: 0;'):
        
        # 消息展示区域 - 占据剩余空间，可滚动，底部留出空间给输入框
        with ui.column().classes('flex-grow overflow-y-auto p-4 w-full rounded').style(
            'scroll-behavior: smooth; min-height: 0; margin-bottom: 80px;'
        ) as scroll_area:
            messages = ui.column().classes('w-full')
            
            # 欢迎消息容器
            welcome_message_container = ui.column().classes('flex items-center justify-center h-full')
        ...
        # 输入区域 - 固定在底部，距离底部10px
        with ui.row().classes('w-full items-center gap-2 p-3 rounded bg-white shadow-lg').style(
            'position: fixed; bottom: 10px; left: 10px; right: 10px; z-index: 1000; margin: 0 auto; max-width: calc(100% - 20px);'
        ):
            # 提前声明可变对象，供内部嵌套函数读写
            ...
            async def handle_message():
                ...
                # 滚动到底部
                await asyncio.sleep(0.1)  # 等待DOM更新
